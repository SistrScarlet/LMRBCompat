...
public class ConfigHubScreen extends Screen {
        ...
        for (Map.Entry<String, ConfigScreenInfo> entry : map.entrySet()) {
            ...
            var builder =
                    new ButtonWidget.Builder(
                            title, (button) -> this.client.setScreen(info.getScreen(this)));
            builder.position(left + (index % 3) * buttonWidth, top + (index / 3) * buttonHeight);
    ...
    protected ButtonWidget createBackButton() {
        var builder =
                new ButtonWidget.Builder(
                        Text.of("back"), (button) -> this.client.setScreen(this.parentScreen));
