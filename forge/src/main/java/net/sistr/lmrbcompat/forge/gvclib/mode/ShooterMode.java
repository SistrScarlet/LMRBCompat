...
public class ShooterMode extends AbstractShooterMode<ItemGunBase> {
    ...
    protected void playReloadStartSound() {
        var soundEvent =
                ForgeRegistries.SOUND_EVENTS.getValue(
                        Identifier.of(weapon.gun_mod_id, weapon.reload_sound));
        if (soundEvent == null) {
            return;
        }
        this.maid
    ...
    protected void fireBullet(ItemStack stack, World worldIn, LivingEntity entity) {
        if (!weapon.isReload(stack)) {
            ...
            SoundEvent gun =
                    ForgeRegistries.SOUND_EVENTS.getValue(
                            Identifier.of(weapon.gun_mod_id, weapon.fire_sound));
            if (supp != null) {
                SoundEvent gunsupp =
                        ForgeRegistries.SOUND_EVENTS.getValue(
                                Identifier.of(weapon.gun_mod_id, weapon.fire_sound_supp));
                if (gun != null) {
                    worldIn.playSound(
                            null,
                            entity.getX(),
                    ...
                    worldIn.playSound(
                            ...
                            entity.getY(),
                            entity.getZ(),
                            GVCSoundEvent.getSound(weapon.fire_sound_supp),
                            SoundCategory.NEUTRAL,
                            3.0F,
                            1.0F);
                }
                ...
                worldIn.playSound(
                        ...
                        entity.getY(),
                        entity.getZ(),
                        GVCSoundEvent.getSound(weapon.fire_sound),
                        SoundCategory.NEUTRAL,
                        3.0F,
                        1.0F);
            }
            ...
            for (int pe = 0; pe < weapon.pellet; ++pe) {
                ...
                bullet.setVelocity(
                        entity, entity.getPitch(), entity.getYaw(), 0.0F, weapon.speed, bbure);
                if (!worldIn.isClient) {
            ...
            reco -=
                    (MathHelper.sin(entity.headYaw * ((float) Math.PI / 180F) + xz)
                            * weapon.fire_posx);
            xro +=
                    (MathHelper.cos(entity.headYaw * ((float) Math.PI / 180F) + xz)
                            * weapon.fire_posx);
            yy11 =
                    (double) MathHelper.sqrt((float) (zzz * zzz))
                            * Math.tan(Math.toRadians(-entity.getPitch()))
                            * (double) 1.0F;
            if (supp == null) {
                ...
                flash.refreshPositionAndAngles(
                        entity.getX() + reco,
                        entity.getY() + (yy - (double) 0.0F) + yy11,
                        entity.getZ() + xro,
                        entity.headYaw,
                        entity.prevPitch);
