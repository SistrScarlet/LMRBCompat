...
Minecraft mod (LMRBCompat) のリリース作業を実行する。
...
gh release create v{version} \
  --title "v{version}" \
  -n "{リリースノート}" \
  fabric/build/libs/LMRBCompat-{mc_version}-{version}-Fabric.jar \
  forge/build/libs/LMRBCompat-{mc_version}-{version}-Forge.jar
```
...
- JARファイル名は `LMRBCompat-{mc_version}-{version}-Fabric.jar` / `LMRBCompat-{mc_version}-{version}-Forge.jar`
